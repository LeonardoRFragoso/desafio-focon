...
describe('TimeEntryDetailsModal — future legacy behavior', () => {
  ...
  it('shows future warning banner for future legacy pending entry', () => {
  ...
  it('disables approve button for future legacy entry', () => {
    const futureDate = '2099-12-31';
    renderModal({ entry: makeEntry({ entry_date: futureDate, approval_status: 'pending' }) });
  ...
  it('keeps reject button enabled for future legacy entry', () => {
    const futureDate = '2099-12-31';
    renderModal({ entry: makeEntry({ entry_date: futureDate, approval_status: 'pending' }) });
  ...
  it('does NOT show future banner for normal past entry', () => {
  ...
  it('enables approve button for normal past entry', () => {
    renderModal({ entry: makeEntry({ entry_date: '2024-01-15', approval_status: 'pending' }) });
  ...
  it('calls onApprove when approve button is clicked for normal entry', async () => {
    const onApprove = vi.fn().mockResolvedValue(undefined);
    renderModal({ entry: makeEntry({ entry_date: '2024-01-15' }), onApprove });
  ...
  it('does not show future banner for future entry that is already rejected', () => {
    const futureDate = '2099-12-31';
